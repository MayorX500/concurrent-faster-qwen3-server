use qwen3_tts::{Language, Qwen3TTS, SynthesisOptions};
...
fn main() {
    ...
    for run in 0..3 {
        let t0 = Instant::now();
        let (audio, timing) = model
            .synthesize_with_timing(text, qwen3_tts::Speaker::Serena, Language::Spanish, None)
            .unwrap();
        let dur = audio.samples.len() as f32 / audio.sample_rate as f32;
        if run == 2 {
            println!(
                "Single: {:.2}s audio in {:.2}s",
                dur,
                t0.elapsed().as_secs_f32()
            );
            println!("  Prefill:    {:.1}ms", timing.prefill_ms);
            println!(
                "  Generation: {:.1}ms ({} frames, {:.1}ms/frame)",
    ...
    let mut stream = model
        .synthesize_streaming(
            "Buenos días, le habla el asistente.",
